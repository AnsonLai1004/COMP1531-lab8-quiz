 ...
 * and there may be code duplications.
 * You should aim to avoid this!
 */
...
function requestHelper(method: HttpVerb, path: string, payload: object) {
  ...
  const res = request(method, `http://localhost:${PORT}` + path, { qs, json });
...
function questionEdit(questionId: number, questionString: string, questionType: string, answers: Answer[]) {
  const res = requestHelper('PUT', '/question/edit', { questionId, questionString, questionType, answers });
  // FIXME 22T3: allow only PUT requests
  return res !== 404 ? res : requestHelper('POST', '/question/edit', { questionId, questionString, questionType, answers });
}
...
describe('/question/add', () => {
  describe('error', () => {
    ...
    describe.each([
      'single', 'multiple'
    ])('Common answers error, type %s', (type) => {
      beforeEach(() => {
        clear();
      });
      test.each(commonAnswerErrors)('invalid answers: $testName', ({ answers }) => {
        const quiz = quizCreate('q', 'q');
        expect(questionAdd(quiz.quizId, '?', type, answers)).toEqual(400);
      });
    });
...
describe('/question/edit', () => {
  describe('error', () => {
    ...
    describe.each(['single', 'multiple'])('Common answers error, type %s', (type) => {
      test.each(commonAnswerErrors)('invalid answers: $testName', ({ answers }) => {
        const quiz = quizCreate('q', 'q');
        const question = questionAdd(quiz.quizId, 'valid', 'single', [validTrueAnswer]);
        expect(questionEdit(question.quizId, '?', type, answers)).toEqual(400);
      });
    });
